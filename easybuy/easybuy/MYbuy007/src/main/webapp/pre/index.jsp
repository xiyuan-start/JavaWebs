...
<head>
    ...
    <script type="text/javascript">
        ...
        function queryProductDeatil(Productid) {
            ...
            var url = "http://192.168.137.100/log.gif?UserId="+Userid+"&action=queryProduct&remark="+"ProductId:"+Productid;
            xmlhttp.open("GET", url, true); //第三个参数是同步异步,主线程只能异步
        xmlhttp.send();
        }

        function queryProductList(productCategoryId) {
            var Userid=<%=session.getAttribute("Userid")%>
            if(Userid==null)
            {
                var Userid=0;
            ...
            var xmlhttp = new XMLHttpRequest();
            var url = "http://192.168.137.100/log.gif?UserId="+Userid+"&action=queryProductList&remark="+"productCategoryId:"+productCategoryId;
            xmlhttp.open("GET", url, true); //第三个参数是同步异步,主线程只能异步
            xmlhttp.send();
        }
...
<div class="menu_bg">
    <div class="menu">
                ...
                <ul>
                    <c:forEach items="${productCategoryVoList}" var="temp" >
                        ...
                        <span class="n_img"><span></span>
                            <img src="http://47.94.101.75/images/${temp.productCategory.iconClass}"/></span>
        ...
        <ul class="menu_r">
            ...
            <c:forEach items="${productCategoryVoList}" var="temp">
                <li onclick="queryProductList(${temp.productCategory.id})"><a href="${ctx}/Product?action=queryProductList&level=1&category=${temp.productCategory.id}">${temp.productCategory.name} </a></li>
            </c:forEach>
        </ul>
                        ...
                        <li>
                            ...
                            <div class="img">
                                <a href="${ctx}/Product?action=queryProductDeatil&id=${productVo.id}">
                                    <img src="http://47.94.101.75/images/${productVo.fileName}" width="185"  height="155" onclick="queryProductDeatil(${productVo.id})"/>
                                </a>
                            </div>
                        </li>
                    </c:forEach>
